...
using static EmployeeManager_Mvc_Modal.Helpers.UIHelper;
    ...
    public class EmployeeController : Controller
    {
        ...
        public async Task<IActionResult> AddOrEdit(IFormFile file, Employee employee)
        {
            if (ModelState.IsValid)
            {
                ...
                var model = await _dbContext.Employees.Include(x => x.Department).ToListAsync();
                return Json(new { isValid = true, html = Helpers.UIHelper.RenderRazorViewToString(this, "_ViewAll", model) });
            }
            return Json(new { isValid = false, html = Helpers.UIHelper.RenderRazorViewToString(this, "AddOrEdit", employee) });
        }
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            await _dbContext.SaveChangesAsync();
            var model = await _dbContext.Employees.Include(x => x.Department).ToListAsync();
            return Json(new { html = Helpers.UIHelper.RenderRazorViewToString(this, "_ViewAll", model) });
        }
    }
}
